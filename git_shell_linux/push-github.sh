...
if timeout 10 git ls-remote --exit-code --heads origin "$branch" >/dev/null 2>&1; then
    remote_branch_exists=0
    echo -e "\033[32m[push-github] 远端分支 $branch 已存在，正在获取最新信息...\033[0m"
    if ! timeout 15 git fetch origin "$branch" 2>/dev/null; then
        echo -e "\033[33m[push-github] git fetch 超时或失败，将继续尝试推送...\033[0m" >&2
    fi
else
